...
type Category =
  RouterOutputs["salesCommissionsV2"]["setup"]["categories"][number];

export function SalesCommissionV2CategoryDialog({
  clientId,
  category,
}: {
  clientId: string;
  ...
  const create = api.salesCommissionsV2.createCategory.useMutation({
    onSuccess: close,
    onError: (value) => setError(value.message),
  });
  const update = api.salesCommissionsV2.updateCategory.useMutation({
    onSuccess: close,
    onError: (value) => setError(value.message),
  });
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Canonical categories define exact matches and commission columns.
          </DialogDescription>
        </DialogHeader>
        <form
          className="space-y-4"
